...
function Ant(crslId, elemsCount, arrows) {
	...
	// Carousel objects
	this.crslList = this.crslRoot.querySelector('.carousel__list');
	this.crslElements = this.crslList.querySelectorAll('.carousel__el');
	this.crslElemFirst = this.crslList.querySelector('.carousel__el');
	this.leftArrow = this.crslRoot.querySelector('.carousel__control#left');
	this.rightArrow = this.crslRoot.querySelector('.carousel__control#right');

	// Initialization
	this.options = Ant.defaults;
    this.options.elemVisible = elemsCount
...
Ant.prototype.dotOff = function(num) {
    if (this.options.dots)
	    this.indicatorDotsAll[num].style.cssText = 'background: #FE8624; cursor:default;'
};
...
Ant.initialize = function(that) {
	...
	if(that.options.dots) {  // инициализация индикаторных точек
		...
		for(let i=0; i<that.dotsVisible; i++) {
			sum += '<div class="home__dot carousel__dot"></div>'
		};
		that.indicatorDots.innerHTML = sum;
		that.indicatorDotsAll = that.crslRoot.querySelectorAll('.carousel__dot');
...
$(document).ready(function() {
	...
	for([index, tabEl] of tabEls.entries()) {
		tabEl.addEventListener('click', function () {
			// Измение переключателя визуально
			for(tab of tabEls) {
